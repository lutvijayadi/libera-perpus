...
$id = $_GET['judul_buku'];
$query = mysqli_query($koneksi, "SELECT * FROM buku WHERE judul_buku = '$id'");
    ...
    <div class="max-w-lg mx-auto bg-white p-8 rounded-xl shadow-md">
        ...
        <form action="../aksi/aksi_edit_buku.php" method="post" enctype="multipart/form-data" class="space-y-4">
            <div>
                ...
                <input type="text" name="judul_buku" value="<?php echo $data['judul_buku']; ?>" required class="w-full p-2 border rounded-md">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium">Pengarang</label>
                    <input type="text" name="pengarang" value="<?php echo $data['pengarang']; ?>" required class="w-full p-2 border rounded-md">
                </div>
                <div>
                    <label class="block text-sm font-medium">Stok</label>
                    <input type="number" name="stok" value="<?php echo $data['stok']; ?>" required class="w-full p-2 border rounded-md">
                </div>
            </div>
            ...
            <button type="submit" name="update" class="w-full bg-green-600 text-white py-2 rounded-md font-bold hover:bg-green-700">Update Buku</button>
        </form>
    </div>
</body>
</html>