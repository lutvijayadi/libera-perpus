...
if (isset($_POST['submit'])) {
    ...
    $new_file_name = time() . "_" . preg_replace("/[^a-zA-Z0-9]/", "_", $judul) . "." . $file_extension;
    ...
    if (move_uploaded_file($tmp_name, $target_file)) {
        // 3. Query INSERT (SESUAIKAN NAMA KOLOM DENGAN DATABASE ANDA)
        // id_buku tidak perlu dimasukkan jika AUTO_INCREMENT
        $query = "INSERT INTO buku (id_buku, judul_buku, pengarang, penerbit, tahun_terbit, stok, cover) 
                  VALUES ('$id_buku', '$judul_buku', '$pengarang', '$penerbit', '$tahun_terbit', '$stok', '$new_file_name')";

        if (mysqli_query($db, $query)) {
            header("location:../admin/dashboard.php?pesan=berhasil");
            exit();
        } else {
